...
async fn handle_entity_recall(
    ...
    let embed_cfg = embed_cfg
        .ok_or_else(|| exec_err("entity_recall: no AI embedding provider configured"))?;
    let embedder = build_embedding_provider(&embed_cfg).map_err(exec_err)?;

    // 5x oversample so a multi-block entity isn't squeezed out when
    // the top-N has multiple chunks from the same file.
    ...
    // Group by file, keep max score, then drop non-entities.
    ...
    // Resolve each surviving entity through storage::entity_get.
    ...
    for (path, score) in ranked {
        ...
        let Some(obj) = resp.get("entity").and_then(serde_json::Value::as_object) else {
            // Stem isn't in the entity index (file lacks `entity_type`
            // frontmatter). Skip silently — recall must not surface
            // arbitrary `entities/` files that aren't entities.
            continue;
        };
        let id = obj
            .get("id")
...
async fn handle_enrich_entity(
    ...
    if let Some(embed_cfg) = embed_cfg {
        if let Ok(embedder) = build_embedding_provider(&embed_cfg) {
            ...
            {
                for hit in matches {
                    // Skip the entity's own files — we want context from
...
async fn handle_infer_entity_relations(
    ctx: &KernelPluginContext,
    ai_cfg: Option<AiConfig>,
    embed_cfg: Option<AiConfig>,
    args: &serde_json::Value,
    ...
    // Existing edge set for fast dedup on (target, kind).
    ...
    let mut neighbours: Vec<(String, String, String)> = Vec::new(); // (id, type, description)
    if let Some(embed_cfg) = embed_cfg {
        if let Ok(embedder) = build_embedding_provider(&embed_cfg) {
    ...
    // Fallback: list a few arbitrary entities so the model has *something*
    // to work with when the embedder is offline.
    ...
    // Models often wrap JSON in prose or code fences — extract the array.
    ...
    for item in parsed_array {
        ...
        // Storage's `entity_upsert` runs the canonical normaliser on write —
        // pass the model's kind through verbatim (after a cheap snake-case
        // normalise so duplicate-check on (target, kind) is stable).
        let canonical = kind
            .to_ascii_lowercase()
            .replace([' ', '-'], "_");
...
fn extract_json_array(reply: &str) -> Option<Vec<serde_json::Value>> {
    // Quick path: the whole reply is JSON.
