...
// ── Types ────────────────────────────────────────────────────────────────────
//
// The shared shape lives in `nexus_types::bases` so that non-storage
// consumers (e.g. `nexus-database`) can reference base values without taking
// a hard dep on this crate.

pub use nexus_types::bases::{
    Base, BaseMetadata, BaseRecord, BaseRelation, BaseSchema, BaseSummary, BaseView, FilterRule,
    SortRule, ViewType,
};
