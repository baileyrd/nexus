...
async fn handle_open_excerpts(
    ...
    for item in &merged {
        ...
        let source =
            crate::handlers::save::read_source_for_excerpts(forge_root, ctx.as_deref(), &item.relpath)
                .await?;
...
async fn handle_refresh_excerpts(
    ...
    for src in &sources_to_read {
        let text =
            crate::handlers::save::read_source_for_excerpts(forge_root, ctx.as_deref(), src).await?;
    ...
    let (value, revision) = {
        ...
        for id in s.tree.root_blocks.clone() {
            ...
            let relocated = if !block.content.is_empty() {
                crate::handlers::save::relocate_excerpt_by_content(source, &block.content)
            } else {
                None
            };
