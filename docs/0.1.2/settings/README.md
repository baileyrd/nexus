...
| **sandbox.toml** | `<forge>/.forge/sandbox.toml` | `SandboxConfig` (`policy` = `SandboxPolicy`, `downloads` = `DownloadPolicy`) — `nexus-security/src/sandbox_config.rs` | `SandboxConfig::load` — same file. See [`os-sandbox.md`](../os-sandbox.md). Defaults closed (read-only, downloads off) |
