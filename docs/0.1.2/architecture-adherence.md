...
| **AA-03** | Open an ADR titled *"CLI-scope exceptions to IPC-only"* documenting the three `nexus-cli` direct uses (`nexus-terminal`, `nexus-collab`, `nexus-security`) with the Phase-H migration plan. | S | architecture |
...
| **AA-06** | Add a rationale comment to `crates/nexus-crdt/Cargo.toml` for the `nexus-editor` dep (currently the only undocumented cross-service dep). | XS | crdt maintainer |
