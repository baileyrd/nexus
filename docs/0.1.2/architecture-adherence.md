...
| ~~**AA-04**~~ | ~~Fix `shell/src/shell/App.tsx:8` — invert the dependency on `plugins/nexus/workspace/workspaceStore`.~~ ✅ **Resolved (P3-03)** — `App.tsx` now reads `rootPath` via `useContextKey('nexus.workspace.rootPath')`, a shell-owned ContextKeyService key the workspace plugin publishes on every `setRoot`. The imports at lines 8-9 (`../workspace/WorkspaceRenderer`, `../workspace/workspaceStore`) are host code (`shell/src/workspace/`), not the plugin store (`shell/src/plugins/nexus/workspace/`); they are permitted. See in-source comment at `App.tsx:25-34`. | M | ✅ |
