...
- **`nexus-storage` Cargo deps on `nexus-database` + `nexus-formats` are intentional, not a leak.** Both target crates explicitly declare themselves as pure-logic libraries (`nexus-formats/src/lib.rs:5`: "Provides pure-Rust parsers and serializers… No runtime services; no SQLite." / `nexus-database/src/lib.rs:3`: "This crate is **pure-logic** — it does not touch `rusqlite`. The SQL-backed query engine, schema migrations, and relation/rollup resolution that previously lived here moved into `nexus-storage`"). The architecture intentionally puts `nexus-storage` ABOVE these libraries — it owns the SQLite backend and consumes their pure-logic types (`DatabaseError`, `PropertyConfig`, `FormulaValue`) and parsers (canvas, config, sha256_hex). The IPC seam is for inter-PLUGIN dispatch, not inter-CRATE library reuse; `dep_invariants.rs::FORBIDDEN` enforces the opposite direction (no `rusqlite` in `nexus-database`). The initial framing of this dependency as a "leak" in earlier drafts was a misreading.
...
8. ~~Move `nexus-storage` compile-time deps on `nexus-database` and `nexus-formats` behind feature flags or extract the shared types to `nexus-types`.~~ **Withdrawn.** Code-level review (Phase 4.8) found this is an intentional pure-logic-library dependency, not a leak — both target crates explicitly declare themselves as bottom-tier libraries, and `dep_invariants.rs` enforces the layering. The IPC seam is for cross-plugin dispatch, not cross-crate reuse. See §6 above for the corrected analysis.
