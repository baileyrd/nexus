...
| `stream_ask` / `ask` / `semantic_search` / `embed_text` / `enrich_file` / `enrich_entity` / `infer_entity_relations` / `generate_docs` / `predict` | `ai.chat` | — | one provider round-trip per call |
