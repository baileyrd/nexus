...
| **SD-04** | ~~Split `PluginContext` into supertraits (`Identity` + `FileSystem` + `KvAccess` + `Events` + `Ipc` + `Log`) with the current trait kept as auto-impl umbrella. Lets pure-compute plugins depend on `Identity + Log`.~~ **Resolved 2026-05-18.** `nexus_kernel::context` now defines the six narrower traits and keeps `PluginContext` as an umbrella supertrait with a blanket impl. `KernelPluginContext` implements each supertrait individually; consumers that continue to take `&dyn PluginContext` are unaffected. Handlers/helpers that only need a slice can now depend on the narrowest one (`&dyn Log`, `&dyn Ipc`, …). | ISP | M | M |
...
**I** ~~is a deliberate trade — the wide `PluginContext` is convenient for plugin authors at the cost of test-mock weight. Splittable into supertraits without breaking callers.~~ Split into `Identity` + `FileSystem` + `KvAccess` + `Events` + `Ipc` + `Log` supertraits per **SD-04**, with `PluginContext` kept as the umbrella alias.
