...
**Bundled shell (RFC 0002):** for *sandboxed* sessions, the terminal can run the bundled, dependency-light [`nexus-rush`](../../crates/nexus-rush/) shell instead of the detected system shell — a Nexus-owned, auditable `/bin/sh` for the confined environment. It is opt-in (`sandbox.toml` → `bundled_shell_for_sandbox`, threaded to `SessionConfig.bundled_shell`) and only applies when a confining policy is set and no shell is pinned; the system `$SHELL` stays the default everywhere else. Bundled rush composes *inside* the `nexus-sandbox` wrapper exactly like system bash. See [RFC 0002](rfcs/0002-bundled-shell-rush.md).
