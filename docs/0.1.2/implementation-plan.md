...
> **Phase 4 substantially complete 2026-05-18.** Items P4-01..P4-05, P4-08, P4-09, P4-10 shipped. P4-06 80/94 (78 settings wired through configStore + Help/Rebuild-cache actions). P4-07 10/12 (split/popout/move/rename/bookmark/linkedView/backlinks-focus/versionHistory/addProperty plus a new `workspace.splitLeaf` primitive, new `storage::write_frontmatter` IPC handler with YAML splice + 6 unit tests, and new `git::file_log` IPC handler). Only `mergeFile` (needs a 3-pane conflict resolver UI) and `exportPdf` (needs a PDF library) remain in P4-07; 14 bespoke action buttons remain in P4-06 (color picker, theme/font/ribbon modals, sync sign-up, etc.).
...
| ID | Item | Effort | Notes |
|----|------|:------:|-------|
| **P4-06** ◐ | Wire the **57 "Coming soon" controls** in `SettingsPanelView.tsx` to real backend handlers. Group by tab: General (5), Editor (21), Files & links (14), Keychain (1), Canvas (8), Backlinks (1), Daily notes (3), File recovery (4), Note composer (4). **Now also includes** the P2-01/02/03 cascade-key exposure (keybinding UI already exists via WI-04; priority + fileExtensions need new UI). | XL | **80/94 shipped** as of 2026-05-18. Added 5 reusable `Wired*` primitives (`WiredToggle`, `WiredSelect`, `WiredNumberRange`, `WiredText`, `WiredNumber`) that read/write per-forge `configStore` under `nexus.settings.*` keys (round-tripping through `<forge>/.forge/app.toml [settings]`). Converted **78 stub controls** across 12 tabs (General, Editor, Files & links, Backlinks, Canvas, Daily notes, File recovery, Note composer, Page preview, Quick switcher, Templates, and the Interface/Font/Advanced sections of Appearance) — values persist across sessions/forges. Two action-buttons also wired: **Help** (opens the GitHub README) and **Rebuild forge cache** (invokes `com.nexus.storage::rebuild_index`). **14 remaining "Coming soon" entries are bespoke action triggers** (Spellcheck options, Excluded files, Add secret, View/Clear snapshots, Sync sign up/log in, Accent color, Manage ribbon, Manage themes, Manage fonts ×3, Custom app icon) — these need their own dedicated modals (color picker, theme browser, font selector, file dialog) and don't fit the "save a value" pattern. |
| **P4-07** ◐ | Wire the **12 editor tab-action stubs** (`nexus.editor.stub.*`): splitRight, splitDown, openInNewWindow, openLinkedView, rename, moveTo, bookmark, addProperty, backlinksInDocument, versionHistory, mergeFile, exportPdf. Each is a discrete handler. | L | **10/12 shipped** as of 2026-05-18: `bookmark`, `rename`, `moveTo`, `backlinksInDocument`, `openInNewWindow`, `openLinkedView`, `versionHistory` (new `com.nexus.git::file_log` IPC handler + `api.input.pick` commit-list modal that copies the picked hash to clipboard), `addProperty` (new `com.nexus.storage::write_frontmatter` IPC handler with full unit-tested YAML splice — insert/replace/remove top-level scalar keys, gracefully handles files without a `---` block), `splitRight`/`splitDown` (new `workspace.splitLeaf(leafId, direction)` primitive that wraps the parent Tabs in a fresh Split or extends an existing same-direction Split — the editor plugin's `mountFileInMainLeaf` then picks up the new empty active leaf for the duplicate-file emit). Remaining 2 deferred: `mergeFile` (needs conflict-list IPC + 3-pane resolver UI — ~150 lines TS), `exportPdf` (needs PDF library dep — `printpdf` or wkhtmltopdf — and a rendering pipeline). |
