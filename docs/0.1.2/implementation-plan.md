...
| **P2-06** ⚠️ partial | User-facing timeouts → settings: `mcp.{connect,shutdown,oauth}_timeout_secs`, `collab.{handshake,backoff_max}_timeout_secs`, `git.{poll_interval,auto_commit_tick}_secs`, `ui.theme_debounce_ms`, `ai.indexing_debounce_secs`. | M | Shipped 2026-05-17 — pragmatic split. Every literal now exposed as a named `pub const DEFAULT_*` (grep-discoverable single source). Audio + collab bootstrap got live `[audio] / [collab]` config-file knobs (`creds_lookup_timeout_secs`, `backoff_factor`, `initial_delay_ms`, `max_delay_ms`, `buffer_capacity`, `handshake_timeout_secs` forward-compat). MCP / git / storage / theme / ai still consult only the const today — adding `[mcp.timeouts]` / `[git.timing]` / `[ai.daemon]` config blocks + thread-through is the deferred follow-up. |
