...
> **Phase 4 partial 2026-05-18.** Items P4-01..P4-05, P4-08, P4-09, P4-10 shipped. P4-06 substantially complete (78/94 stubs wired to configStore; remaining 16 are non-settable action buttons awaiting backend modals). P4-07 partial (6/12 tab-action stubs wired). Remaining surface is genuinely XL — bespoke modals / new IPC handlers / PDF library — and warrants its own dedicated PRs.
...
| ID | Item | Effort | Notes |
|----|------|:------:|-------|
| **P4-06** ◐ | Wire the **57 "Coming soon" controls** in `SettingsPanelView.tsx` to real backend handlers. Group by tab: General (5), Editor (21), Files & links (14), Keychain (1), Canvas (8), Backlinks (1), Daily notes (3), File recovery (4), Note composer (4). **Now also includes** the P2-01/02/03 cascade-key exposure (keybinding UI already exists via WI-04; priority + fileExtensions need new UI). | XL | Partial 2026-05-18. Added 5 reusable `Wired*` primitives (`WiredToggle`, `WiredSelect`, `WiredNumberRange`, `WiredText`, `WiredNumber`) that read/write per-forge `configStore` under `nexus.settings.*` keys (round-tripping through `<forge>/.forge/app.toml [settings]`). Converted **78 of the original 94 stub controls** across 12 tabs (General, Editor, Files & links, Backlinks, Canvas, Daily notes, File recovery, Note composer, Page preview, Quick switcher, Templates, and the Interface/Font/Advanced sections of Appearance). Values now persist across sessions/forges; consumer code in feature plugins can read the same key when each behavior ships. **16 remaining "Coming soon" entries are action buttons** (Help, Spellcheck options, Excluded files, Rebuild forge cache, Add secret, View/Clear snapshots, Sync sign up/log in, Accent color, Manage ribbon, Manage themes, Manage fonts ×3, Custom app icon) — these aren't value-settable and need bespoke modals or external workflows when their backends ship. |
| **P4-07** ◐ | Wire the **12 editor tab-action stubs** (`nexus.editor.stub.*`): splitRight, splitDown, openInNewWindow, openLinkedView, rename, moveTo, bookmark, addProperty, backlinksInDocument, versionHistory, mergeFile, exportPdf. Each is a discrete handler. | L | Partial 2026-05-18: **6/12 shipped** — `bookmark` (→ `nexus.bookmarks.toggleActive`), `rename` and `moveTo` (→ `storage::rename_entry` via `api.input.prompt` — moveTo prompts for a full forge-relative path), `backlinksInDocument` (→ `nexus.backlinks.focus`), `openInNewWindow` (→ `popoutWindowBridge.popoutLeaf(activeLeafId)`), `openLinkedView` (→ `storage::backlinks` + `api.input.pick` + `files:open`). Remaining 6 deferred: `splitRight`/`splitDown` (need a new `workspace.splitLeaf(leafId, direction)` primitive — ~40 lines but touches layout invariants), `addProperty` (needs new `storage::write_frontmatter` IPC handler), `versionHistory` (needs git file-log IPC + history UI), `mergeFile` (needs conflict-list IPC + 3-pane resolver), `exportPdf` (needs PDF library — `printpdf` or wkhtmltopdf). |
