...
> **Phase 4 complete 2026-05-18.** P4-01..P4-05, P4-08, P4-09, P4-10 shipped in full. P4-06 88/94: 78 settings round-trip through configStore; 10 action buttons wired (Help, Rebuild cache, Add secret, Sync sign-up/log-in, Accent color, three font stacks, Custom app icon). P4-07 11/12: split/popout/move/rename/bookmark/linkedView/backlinks-focus/versionHistory/addProperty/exportPdf all wired, backed by a new `workspace.splitLeaf` primitive, new `storage::write_frontmatter` IPC (unit-tested), and new `git::file_log` IPC. Only `mergeFile` (needs a 3-pane CodeMirror merge view) and 6 settings entries (Spellcheck-options sub-modal, Excluded-files glob-list editor, View/Clear file-recovery snapshots, Manage-themes browser, Ribbon-menu configuration) remain — each requires its own subsystem, not a one-line wire-through.
...
| ID | Item | Effort | Notes |
|----|------|:------:|-------|
| **P4-06** ◐ | Wire the **57 "Coming soon" controls** in `SettingsPanelView.tsx` to real backend handlers. Group by tab: General (5), Editor (21), Files & links (14), Keychain (1), Canvas (8), Backlinks (1), Daily notes (3), File recovery (4), Note composer (4). **Now also includes** the P2-01/02/03 cascade-key exposure (keybinding UI already exists via WI-04; priority + fileExtensions need new UI). | XL | **88/94 shipped** as of 2026-05-18. Added 5 reusable `Wired*` primitives (`WiredToggle`, `WiredSelect`, `WiredNumberRange`, `WiredText`, `WiredNumber`) plus `WiredAccentColor` and `CustomAppIconChooser` that read/write per-forge `configStore` under `nexus.settings.*` keys (round-tripping through `<forge>/.forge/app.toml [settings]`). Converted **78 stub controls** across 12 tabs (General, Editor, Files & links, Backlinks, Canvas, Daily notes, File recovery, Note composer, Page preview, Quick switcher, Templates, and the Interface/Font/Advanced sections of Appearance). Plus **10 wired action buttons**: Help (GitHub README), Rebuild forge cache (`com.nexus.storage::rebuild_index`), Add secret (`com.nexus.security::set_secret` via 3 prompts), Sync sign-up/log-in (external URLs), Accent color (color input → configStore), Interface/Text/Monospace fonts (text inputs → configStore CSS font-family stack), Custom app icon (`openDialog` → configStore path). **6 remaining "Coming soon" entries** each need their own subsystem: Spellcheck options sub-modal, Excluded files glob-list editor, View/Clear file-recovery snapshots (snapshot subsystem not built), Manage themes (community theme browser), Ribbon menu configuration. |
| **P4-07** ◐ | Wire the **12 editor tab-action stubs** (`nexus.editor.stub.*`): splitRight, splitDown, openInNewWindow, openLinkedView, rename, moveTo, bookmark, addProperty, backlinksInDocument, versionHistory, mergeFile, exportPdf. Each is a discrete handler. | L | **11/12 shipped** as of 2026-05-18: `bookmark`, `rename`, `moveTo`, `backlinksInDocument`, `openInNewWindow`, `openLinkedView`, `versionHistory` (new `com.nexus.git::file_log` IPC handler + `api.input.pick` commit-list modal that copies the picked hash to clipboard), `addProperty` (new `com.nexus.storage::write_frontmatter` IPC handler with full unit-tested YAML splice — insert/replace/remove top-level scalar keys, gracefully handles files without a `---` block), `splitRight`/`splitDown` (new `workspace.splitLeaf(leafId, direction)` primitive that wraps the parent Tabs in a fresh Split or extends an existing same-direction Split — the editor plugin's `mountFileInMainLeaf` then picks up the new empty active leaf for the duplicate-file emit), `exportPdf` (switches the active tab to preview mode, waits one frame for render, then opens the OS print dialog — the OS-native "Save as PDF" affordance handles the actual conversion). Remaining 1 deferred: `mergeFile` (needs a 3-pane CodeMirror merge view + new IPC handlers for theirs/ours/resolved blobs — ~200 lines of UI). |
