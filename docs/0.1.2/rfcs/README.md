...
| [0008](0008-agent-session-tree.md) | Phase 5.4 — agent session tree | Draft — design | Turn flat single-pass sessions into a resumable tree (resume / branch / rewind / checkpoint). Insight: all three are one `fork(parent, k, message)` primitive; build = a seedable loop + parent linkage. Immutable fork-nodes (delta-stored), non-destructive rewind. Phased PR 1 (resumable loop) → 2 (resume) → 3 (branch/rewind) → 4 (checkpoints + frontends). |
