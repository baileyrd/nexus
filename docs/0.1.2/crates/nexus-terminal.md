...
- **`com.nexus.activity.appended`** (BL-057, kernel-owned topic) — session-boundary events (`SessionCreated` / `SessionClosed` / `MemoryLimitExceeded` / `SoftLimitExceeded`) are also fanned out as `nexus_types::activity::ActivityEntry` (surface `Process`, origin `Terminal(<id>)`) so the BL-052 activity timeline sees terminal activity. Streaming/internal variants (`OutputReceived`, `PatternMatched`, `SessionRenamed`, `SessionEvicted`) intentionally don't reach the activity log. `MemoryLimitExceeded`/`SoftLimitExceeded` reach this fan-out via `memory_poller_round` calling the same `publish_lifecycle_event` every other lifecycle event goes through (#409 — previously the poller published `MemoryLimitExceeded` directly on the bus, bypassing the fan-out, so only the generic `SessionClosed` row ever reached the activity pane).
