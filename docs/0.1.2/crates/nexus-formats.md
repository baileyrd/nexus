...
- `export_to_html(content, title) -> String` — standalone styled HTML doc (safe mode, raw HTML escaped).
...
**HTML export (`html.rs`).** Renders with the same comrak extensions but `render.unsafe = false` (raw HTML escaped). Wraps the body in a complete standalone document with an inlined GitHub-ish stylesheet; the `<title>` is manually HTML-escaped.
...
- `markdown/html.rs` — heading/code/tasklist/table rendering, full doc structure, title escaping.
