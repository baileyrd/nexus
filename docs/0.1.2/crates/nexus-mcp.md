...
The server presents 60 static `nexus_*` tools (the `#[tool]` macros in `server.rs`) plus any dynamically-registered tools, to external MCP clients. (The `server.rs` module doc's "15 tools" headline and this file's previous "19 tools" count are both stale — the table below only ever tracked the original note/graph/RAG/skills surface; it was never extended for the memory, terminal, sandbox, git fetch/pull/remotes, `nexus_export_html`, or `nexus_comment_*` tools added by later findings, and is not reproduced in full here. `grep -oP 'name = "nexus_\w+"' crates/nexus-mcp/src/server.rs | sort -u | wc -l` is the source of truth for the current count.) Each routes through `ipc_call`; on IPC failure most tools return a degraded payload rather than a protocol error.
