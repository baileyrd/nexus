...
The server presents 61 static `nexus_*` tools (the `#[tool]` macros in `server.rs`) plus any dynamically-registered tools, to external MCP clients. (The `server.rs` module doc's "15 tools" headline and this file's previous "19 tools" count are both stale — the table below only ever tracked the original note/graph/RAG/skills surface; it was never extended for the memory, terminal, sandbox, git fetch/pull/remotes, `nexus_export_html`, `nexus_comment_*`, `nexus_agent_*`/`nexus_workflow_*`, or `nexus_semantic_search` tools added by later findings, and is not reproduced in full here. `grep -oP 'name = "nexus_\w+"' crates/nexus-mcp/src/server.rs | sort -u | wc -l` is the source of truth for the current count.) Each routes through `ipc_call`; on IPC failure most tools return a degraded payload rather than a protocol error.

C75 (#428) added `nexus_agent_run` / `nexus_agent_sessions` (→ `com.nexus.agent` `session_run` / `session_list`) and `nexus_workflow_list` / `nexus_workflow_run` (→ `com.nexus.workflow` `list` / `run`) — the MCP server's first reach into either of Nexus's two execution surfaces. `nexus_agent_run` always sets `auto_approve: true`: a synchronous MCP tool call has no bidirectional channel for the CLI's interactive `round_proposed`/`round_decide` approval loop (BL-132), so every tool call in the session auto-approves, same as the CLI's non-interactive default. Both `_run` tools use the extended IPC timeout (`DEFAULT_RUN_IPC_TIMEOUT`, 600s) since they can drive many LLM/tool round-trips; the `_list`/`_sessions` reads use the short default. `nexus_workflow_run` inherits the same per-step (not per-workflow) capability aggregation caveat as the CLI's `workflow run` — see the `com.nexus.workflow::run` AUDIT entry in `docs/0.1.2/reference/audit-flags.md` (issue #77).

C78 (#431) added `nexus_semantic_search` (→ `com.nexus.ai` `semantic_search`) — unlike `nexus_ask`, which synthesizes a chat answer and reports only a source *count*, this returns the raw ranked chunk hits (`file_path`, `score`, `excerpt`/`chunk_text`) for agents/scripts that want retrieval results directly. `hybrid: true` fuses the vector ranking with lexical Tantivy BM25 via RRF (same `com.nexus.storage::hybrid_search` handler behind the shell's "Search by Meaning"); the default is vector-only. Uses `AI_IPC_TIMEOUT` like `nexus_ask`, and requires an AI embedding provider configured (`ai.toml`) — same precondition as `nexus_ask`'s retrieval half.
