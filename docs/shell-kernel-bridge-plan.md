...
Branch: `feat/ui-shell-rebuild`. Pushed through `1ef3d81`; local commits ahead through `7d0ac8b`. Phase 0 (bridge infra) and Phase 1 (retire standalone commands) shipped. Phase 2 is 13 plugins deep — editor (read-only → markdown → tabs → edit/save), command palette, search, outline (+ scroll-spy), backlinks, graph, terminal, AI chat, pane-mode infra, plugins-mgmt, processes, workflows, skills. Phase 3 polish underway: design-bundle layout widths + statusbar height aligned, typed icon module ported. Tree is clean; the React plugin layer now talks to the kernel via `api.kernel.invoke` / `api.kernel.on` for every backend call except the pre-kernel `path_exists` (workspace verification during launcher restore).
...
- **Specialty**: ~~`nexus.mcp`~~ shipped sidebar v1; tool-call modal (args input + result display) is the obvious follow-up.
...
| 17 | `nexus.mcp` | `com.nexus.mcp.host::list_servers/connect/disconnect/list_tools/list_resources/list_prompts` | `activityBar`, `sidebarContent` | — | _(pending commit)_ sidebar v1 — server list + connect/disconnect + capability inspection |
