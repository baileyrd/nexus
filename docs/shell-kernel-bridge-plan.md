...
| 11 | `nexus.agent` | `com.nexus.agent::plan/run/history_list/history_get/history_delete` + `com.nexus.agent.*` topic stream | `paneMode` (full-pane) | forge_orchestrate | `29563bc` paneMode v1 + _(pending)_ history-delete |
...
- _(pending commit)_ — `nexus.agent` history-delete + `trash` icon. Trash button on each history row (also added `trash` to the icon module — Lucide-style). Click → `window.confirm` (matches the close-while-dirty stop-gap) → `com.nexus.agent::history_delete { plan_id }`. If the deleted run was loaded into the right column, the plan/observation/phase reset so we're not pointing at a vanished record. Failures surface via notifications.
