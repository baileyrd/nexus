...
Branch: `feat/ui-shell-rebuild`. Pushed through `1ef3d81`; local commits ahead. Phase 0 (bridge infra) and Phase 1 (retire standalone commands) shipped. Phase 2 is 12 plugins deep — editor (read-only → markdown → tabs → edit/save), command palette, search, outline (+ scroll-spy), backlinks, graph, terminal, AI chat, pane-mode infra, plugins-mgmt, processes, workflows. Tree is clean; the React plugin layer now talks to the kernel via `api.kernel.invoke` / `api.kernel.on` for every backend call except the pre-kernel `path_exists` (workspace verification during launcher restore).
...
| 12 | `nexus.workflow` | `com.nexus.workflow::list/run` | `activityBar`, `sidebarContent` | — | _(pending commit)_ sidebar v1 — list + manual run; full-pane is a follow-up |
