...
| 11 | `nexus.agent` | `com.nexus.agent::plan/run/history_list/history_get` + `com.nexus.agent.*` topic stream | `paneMode` (full-pane) | forge_orchestrate | `29563bc` paneMode v1 — composer + plan + live step status + history |
...
| 17 | `nexus.mcp` | `com.nexus.mcp.host::list_servers/connect/disconnect/list_tools/list_resources/list_prompts` | `activityBar`, `sidebarContent` | — | `29563bc` sidebar v1 — server list + connect/disconnect + capability inspection |
