...
- [x] CM6 breakpoint gutter clickable, syncs through `set_breakpoints`. New `shell/src/plugins/nexus/editor/cm/breakpointGutter.ts` ships a CM6 `gutter()` extension wired into the editor's code-mode bundle (alongside the BL-079 git gutter and BL-077 LSP client). Click toggles via `useDebuggerStore.toggleBreakpoint(api, relpath, line)` — the store records the breakpoint locally and dispatches `set_breakpoints` to the active adapter; clicks without an active session still record so the next `launch` replays cached entries pre-`configurationDone`. The gutter subscribes to the Zustand store via `getSnapshot` + `subscribe` adapters so any source of breakpoint mutation (panel, other tab) keeps every open view's gutter in sync. Pure factor `linesForPath` + `lineSetsEqual` short-circuit unchanged sets so the editor's transaction stream stays quiet on no-op store mutations.
- [x] `pnpm --filter nexus-shell test` stays green.
- [x] Live smoke against a real upstream DAP adapter. `crates/nexus-dap/tests/end_to_end.rs::live_smoke_debugpy_initialize_handshake` spawns Python's `debugpy.adapter` — the same DAP server VS Code uses — completes the `initialize` handshake, asserts upstream-reported capabilities (`supports_configuration_done` + `supports_function_breakpoints`), and disconnects cleanly. Scope is intentionally narrow (handshake + caps + clean exit, no `launch`/breakpoints/`continue`); going further would need a target script + deterministic stop semantics that the mock adapter already exercises exhaustively. The value is confirming that an *unmodified* upstream adapter speaks the same envelope `transport.rs` / `protocol.rs` / `client.rs` assume — guarding against regressions our hand-rolled mock would mirror by construction. Honours `$NEXUS_DAP_LIVE_DEBUGPY_PYTHON` (full path to a venv python with `debugpy` installed); falls back to probing `python3`. Skips silently on machines without `debugpy`. Smoke against `codelldb` / `js-debug` / `delve` remains an operator step since those binaries aren't on every dev box.
