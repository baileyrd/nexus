...
| 09 | Terminal & Process Manager | 🟡 | Phases A–E shipped (`nexus-terminal` crate: PTY primitive, 10 MB byte ring, 50-session manager, INT→TERM→KILL escalation, ANSI-stripped line view with dedup + exact/regex search); env / AI / CLI / process-group kill still pending |
