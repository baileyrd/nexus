...
| 09 | Terminal & Process Manager | 🟡 | Phases A–F shipped (`nexus-terminal` crate: PTY primitive, 10 MB byte ring, 50-session manager, INT→TERM→KILL escalation now signalling the whole process group per §5.2, ANSI-stripped line view with dedup + search); env / AI / CLI / SQLite persistence still pending |
