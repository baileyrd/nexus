...
| 09 | Terminal & Process Manager | 🟡 | Phases A–D shipped (`nexus-terminal` crate: PTY primitive, 10 MB ring buffer, 50-session manager, INT→TERM→KILL signal escalation on Unix); ANSI parsing / env / AI / CLI / process-group kill still pending |
...
**Shipped (Phase D — signal escalation):** `Signal::{Int, Term, Kill}` enum + `Session::send_signal(Signal)` + `Session::request_shutdown(step_timeout)` implementing the PRD-09 §5.1 INT→TERM→KILL ladder via `libc::kill` on Unix; `SessionManager::request_shutdown` passthrough. Returns the signal that actually terminated the child so callers can log escalation level. Windows degrades INT/TERM to force-kill because `portable-pty` doesn't expose softer shutdown — documented inline. 38/38 unit tests pass + 1 `#[ignore]`-gated test for the SIGKILL step (startup race against Python signal-handler install); clippy `-D warnings` clean.
**Gaps (Phase E+):** No process-group / session-leader kill for child-tree termination (§5.2). No Windows Job Objects (§5.3). No ANSI parser / line-indexed view / dedup (§3.2). No SQLite persistence / scrollback on disk / LRU eviction policy (§2.2, §2.3, §13). No explicit process lifecycle state machine (§4). No URL detection (§6). No memory monitoring / global cap (§7, §3.4 pressure). No env-var resolution / `.env` parsing (§8). No compound command splitting (§9). No ad-hoc command system (§10). No programmable terminal API (§11). No AI integration (§12). No terminal UI components (§14). No `nexus proc` / `nexus term` CLI.
