...
> **Snapshot date:** 2026-04-17 (post-session — Chat UI, streaming RAG, inline AI, month-grid Bases calendar, ANSI colour, saved-commands sidebar, block-form MDX all landed)
...
| 10 | Database Engine | 🟢 | `.bases` parse + validate + formula IPC + view application engine; Table/Kanban/Calendar-month-grid/Gallery renderers shipped |
| 11 | Git Integration | 🟢 | 1.1k-line `GitEngine` over `git2`; worker-thread wrapper for UI still needed |
| 12 | AI Engine | 🟢 | Streaming chat + RAG (`stream_chat` / `stream_ask`), Chat panel with system prompt + persisted history, inline editor completion |
...
**Gaps:**
- Nested markdown rendering inside MDX block tags — inner text shows as plain text; feeding the inner range back through the markdown parser is a later slice.
...
**Shipped (Phase X — saved-commands sidebar, commit [55eca8c](https://github.com/baileyrd/nexus/commit/55eca8c)):** [`SavedCommandsPanel`](app/src/components/panels/SavedCommandsPanel.tsx) registered under content-type `com.nexus.terminal.saved-commands` with palette command "Terminal: Saved commands". Add/edit/delete/reorder CRUD over a [`zustand`-backed localStorage store](app/src/stores/savedCommands.ts) shaped to the eventual `procmgr_commands` schema (slug, shell, shell_cmd, working_dir, icon). "Run" sends the stored command into the first live terminal session (spawning one if none exists); "Copy" copies to clipboard.
**Gaps:** No FTS5 scrollback index (§19.3). No `nexus proc` CLI. Saved-commands / ad-hoc CRUD isn't yet exposed through a core plugin (library-only + client-side store for now — schema matches the eventual plugin surface so the UI won't change when it moves). No event streaming over plugin IPC (in-process `std::sync::mpsc` only). No `subscribe_events` dispatch handler pending a plugin-host stream convention.
...
**Gaps:** No `nexus db` CLI. No cross-database relation queries (rollup / lookup resolvers go through `com.nexus.storage`). `BL-002` typed property columns not implemented. Drag-to-reorder and inline-edit affordances on Kanban / Table cells are still read-only.
...
3. **Editor (08) moved 🟡→🟢** with inline AI completion, block-form MDX support, and the existing CM6 surface. Remaining gap is nested-markdown rendering inside MDX blocks — a pleasant polish, not a blocker.
4. **AI (12) moved 🟡→🟢** in a single focused push: streaming chat, streaming RAG, and inline editor completion all ship through the same `com.nexus.ai.stream_*` event plumbing. Microkernel boundary held — all provider dispatch, retrieval, and prompt assembly stay inside `com.nexus.ai`; the shell contributes only Tauri bridges and UI. Remaining polish (source citations, plugin-backed session store, PII filters) is incremental.
5. **Bases (10) moved 🟡→🟢** — all four view renderers now exist, Calendar is a proper month grid. Drag-to-reorder on Kanban and inline-edit on Table are the next usability wins.
6. **Terminal (09) extended with ANSI colour + saved-commands sidebar.** Library surface still feature-complete against the PRD acceptance criteria (239 unit tests); the Tauri panel now renders true-colour output and exposes a CRUD'able command library (client-side store, schema-compatible with the eventual `procmgr_commands` plugin handlers).
7. **Skills / Agents / Workflows (13/15/16) are aspirational.** Specs are good; code is zero. Treat as Phase 2/3, not 1.0 scope.
...
| Client-side storage for Chat + Saved commands | UI state lives in localStorage, not a plugin-backed store | Schemas already shaped to their eventual plugin surfaces (`procmgr_commands` for saved-commands, PRD-12 §8 for chat sessions) so UX won't change when storage migrates. |
