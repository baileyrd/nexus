...
| 09 | Terminal & Process Manager | 🟡 | Phases A–G shipped (`nexus-terminal` crate + `nexus term` CLI with `env` / `run` / `shell` subcommands); daemon-backed multi-session registry, env / AI / SQLite persistence still pending |
