...
| 09 | Terminal & Process Manager | 🟢 | Phases A–V shipped; `com.nexus.terminal` exposes 10 dispatch handlers; Tauri panel renders live ANSI colour; saved-commands sidebar (client-side) live |
...
**Gaps:** No FTS5 scrollback index (§19.3). No `nexus proc` CLI. Ad-hoc CRUD (§10.1) still library-only — not yet exposed through plugin IPC. No event streaming over plugin IPC (in-process `std::sync::mpsc` only). No `subscribe_events` dispatch handler pending a plugin-host stream convention.
...
| Client-side storage for Chat sessions | UI state for chat transcripts + system prompt still lives in localStorage | PRD-12 §8 plugin-backed session store still pending. Saved-commands already migrated to `com.nexus.terminal` in commit 96ffca9, proving the pattern. |
