...
| 09 | Terminal & Process Manager | 🟡 | Phases A–I shipped (`nexus-terminal` crate: PTY, ring buffer, session manager, signal ladder, line view, URL detection, env resolution + `.env` parsing); daemon-backed multi-session registry, AI, SQLite persistence still pending |
...
**Shipped (Phase I — env resolution + `.env` parsing §8):** [`parse_env_file`](crates/nexus-terminal/src/env.rs) / `parse_env_text` return ordered `Vec<(key, value)>` (preserves declaration order for display and `export` semantics), stripping balanced single- or double-quoted values, skipping comments and blank lines, and surfacing duplicates so callers can see conflicts. [`resolve_env`](crates/nexus-terminal/src/env.rs) merges four PRD §8.1 layers (command → `.env` → shell → nexus-injected) with later writes winning on collision and first-sighting preserved. [`interpolate_env`](crates/nexus-terminal/src/env.rs) expands `$VAR` / `${VAR}` references across the merged set up to 10 passes (PRD §8.3), leaving unknown refs literal and surviving cycles. [`is_secret_key`](crates/nexus-terminal/src/env.rs) / [`mask_secrets`](crates/nexus-terminal/src/env.rs) mark values under `*API*`, `*KEY*`, `*SECRET*`, `*TOKEN*`, `*PASSWORD*` (case-insensitive) for redaction in the process panel and logs. 108/108 unit tests pass + 1 ignored; +22 env tests (parse/precedence/interpolation/cycles/secret-detection/masking). Clippy `-D warnings` clean.
**Gaps (Phase J+):** No daemon-backed multi-session registry (name-addressed sessions surviving across CLI invocations — depends on §2.2 SQLite persistence + IPC socket). No Windows Job Objects (§5.3). No scrollback on disk / LRU eviction policy (§2.3). No explicit process lifecycle state machine (§4). No memory monitoring / global cap (§7, §3.4 pressure). No shell-profile sourcing on spawn (§1.3). No compound command splitting (§9). No ad-hoc command system (§10). No programmable terminal API (§11). No AI integration (§12). No terminal UI components (§14). No `nexus proc` CLI (waits on §10 ad-hoc command system). Live stdin forwarding in `nexus term shell` (line-by-line display is enough for verification today).
