...
| 09 | Terminal & Process Manager | 🟡 | Phases A–K shipped (`nexus-terminal` crate + `nexus term` CLI + URL detection + env resolution + `ProcessState` lifecycle + compound-command chain parser); daemon-backed multi-session registry, AI, SQLite persistence still pending |
