...
| 09 | Terminal & Process Manager | 🟡 | Phases A–J shipped (`nexus-terminal` crate + `nexus term` CLI + URL detection + env resolution + `ProcessState` lifecycle); daemon-backed multi-session registry, AI, SQLite persistence, compound commands still pending |
