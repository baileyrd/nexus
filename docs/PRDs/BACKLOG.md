...
- ✅ **WorkspaceEdit applier — rename track.** Shipped 2026-05-09. New `cm/workspaceEdit.ts` module ships `applyWorkspaceEdit(edit, opts)` plus the supporting helpers (`uriToRelpath`, `groupEditsByRelpath`, `applyTextEditsToString`). The applier walks the LSP `WorkspaceEdit.changes` map, maps each URI to a forge-relative path (skips outside-forge URIs with a debug log), and routes per-file slices through either the live CM6 view (active tab — preserves cursor + undo) or `com.nexus.storage::write_file` (every other URI). Bottom-up edit application matches the format-on-save path so earlier edits don't invalidate later positions. New `nexus.editor.lsp.rename` command (bound to F2 — VS Code muscle memory) prompts for the new name (pre-filled with the word at cursor), dispatches `com.nexus.lsp::rename`, applies the WorkspaceEdit, and surfaces a "Renamed in N files" toast. 17 unit tests pin URI normalisation (Linux / Windows / percent-decode / outside-forge), bottom-up apply, mixed live + storage routing, and skip / no-op cases. Code-actions track still deferred — needs a quick-pick UI surface (the InputAPI today exposes only `prompt` / `confirm`); when a list-picker lands, plumbing code-actions onto it is one IPC + one applier call.
