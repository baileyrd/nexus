...
_F-8.1.1 (sub-tasks 1–5: iframe scaffold + sandbox flags, postMessage protocol, `NexusPluginContext` proxy, per-plugin manifest `sandboxed` flag, CSP + tests) shipped 2026-04-28 — see [BACKLOG_COMPLETED.md](BACKLOG_COMPLETED.md). Follow-on item — ship a precompiled `bootstrapSandboxedPlugin` runtime so plugins stop hand-rolling the protocol — tracked as **F-8.1.1-fo1** below._

- [ ] **F-8.1.1-fo1 — Ship the precompiled sandbox runtime bundle.** F-8.1.1 closure left `getRuntimeUrl` in `shell/src/main.tsx:248-256` returning a no-op shim, and `shell/src/plugins/community/hello-world/index.js` hand-rolling the postMessage protocol inline (per its module header). The proxy itself is fully implemented in `packages/nexus-extension-api/src/sandbox/runtime.ts`; the gap is purely the build step that emits a self-contained ESM bundle the iframe can dynamic-import without a module resolver for bare specifiers (`@nexus/extension-api`). Plan: add a Vite library-mode entry that bundles `runtime.ts` + `protocol.ts` into `dist/sandbox-runtime.js`, swap the shim for a fetch + blob-wrap of that asset, then rewrite `hello-world/index.js` as the bundler output of `index.ts` to lock in the migration shape. Small effort (1–3 eng-days). Until this lands, sandboxed community plugins must hand-roll the protocol; the routing path itself is production-ready.
