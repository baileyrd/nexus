...
>
> **Phase 2a (`wait_for` IPC primitive) landed 2026-05-15.** New `HANDLER_WAIT_FOR = 9` on `com.nexus.ai.runtime` blocks until a task hits terminal status (Completed / Failed / Cancelled) or an optional `timeout_ms` elapses; reply carries `{ run, timed_out }` so callers don't have to inspect status against the terminal set. `RunRow` gained a `tokio::sync::Notify` that `observe_status` fires on terminal transitions; the handler uses the standard build-future-before-recheck pattern to avoid lost-wakeup races. Cap-gated on `ai.runtime.observe`. 4 new handler tests + 3 new scheduler tests; 31/31 in-crate tests pass. Foundation for Phase 2b's `delegate` shim refactor.
>
> **Phase 2b-i (`delegate` shim on the runtime) landed 2026-05-15.** `com.nexus.agent::delegate` (HANDLER_DELEGATE = 24) no longer drives the sub-session inline via `handle_session_run`. It now packs the args into `AgentTaskKind::Session`, submits via `com.nexus.ai.runtime::submit`, blocks on `wait_for` with no inner timeout (the runtime + per-round `approval_timeout_secs` already enforce ceilings), then extracts the `Finished.outcome` from the run's terminal event. Wire-level reply shape preserved so `delegate.v1` keeps working without a version bump. `agent_capabilities()` in bootstrap gains `AiRuntimeSubmit` + `AiRuntimeObserve` (no `AiRuntimeControl` — delegate never cancels its child). New pure `extract_session_outcome` projection with 6 unit tests (finished / failed / cancelled / timed-out / no-terminal-event / tail-first-scan-past-token-chunks); the dispatch site drops the `pending_approvals` parameter since the runtime now owns the session execution path. 163/163 agent tests pass, cap_matrix + dep_invariants + agent_ipc integration suites all green.
>
> **Phase 3 (workflow `async = true`) landed 2026-05-15.** New `async: bool` field on `Step` (serde-renamed from `async` since the Rust keyword can't be a field ident). The `ai_prompt` / `ai_decision` / `notify` arms in `KernelActionDispatcher::run` check the flag: when set, the step submits to `com.nexus.ai.runtime::submit` (as an `AgentTaskKind::WorkflowAiStep`) and returns `{ task_id }` as the step's output instead of awaiting the underlying IPC inline. `AgentTaskKind::WorkflowAiStep` reshape: now carries `target_plugin` + `command` + `args` + observability fields (`workflow` = step name, `step` placeholder pending workflow-id threading) so the runtime worker can dispatch a generic `ipc_call(target_plugin, command, args)`. New `WORKFLOW_STEP_TIMEOUT = 5min` on the runtime mirrors `DEFAULT_STEP_TIMEOUT` so async steps don't outlive the inline ceiling. Pure `build_async_submit_args` projection with 4 new tests (parse async = true, default false, envelope shape, unnamed-step workflow tag); runtime gets `workflow_ai_step` arm + adapted variant tests. `workflow_capabilities()` gains `AiRuntimeSubmit` (no observe — the workflow records `task_id` for the user, doesn't poll it). 193/193 workflow tests pass; cap_matrix + dep_invariants green; ts-rs + JSON schemas regenerated.
>
> **Phases 2b-ii / 4 / 5 still open:** Phase 2b-ii (AiEvent correlation — runtime subscribes to `com.nexus.ai.stream_*` / `com.nexus.agent.round_*` and republishes typed envelopes), Phase 4 (indexing-daemon migration), Phase 5 (cancel/pause/resume + `Critical` priority). **Phase 6 (notification router refactor) closed 2026-05-15 via [BL-135](BACKLOG_COMPLETED.md#bl-135-notification-router-refactor--2026-05-15).**
...
**Related**: ADR 0028 (this work), [ADR 0022](../adr/0022-per-handler-ai-capabilities.md) (cap matrix this extends), [ADR 0024](../adr/0024-agent-session-tool-loop.md) (session loop the runtime drives), [BL-135](BACKLOG_COMPLETED.md#bl-135-notification-router-refactor--2026-05-15) (Phase 6 — closed), [BL-138](BACKLOG_COMPLETED.md#bl-138-default-deny-capability-registration--2026-05-15) (pre-req: cap matrix default-deny — closed).
