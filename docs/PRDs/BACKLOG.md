...
Cron + file_event + git_event triggers + condition evaluator + variable interpolation + manual executor + per-step retry/backoff + parallel step scheduler + AI step types are all live. Outstanding: webhook trigger (HTTP listener), mcp_event trigger, and a built-in workflow templates library. Track as one umbrella; mint sub-IDs when prioritised.
