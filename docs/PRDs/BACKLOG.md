...
- [ ] **Settings → Plugins "Enable" on a catalog community plugin fails with "Plugin '<id>' not found".** Surfaced 2026-05-13 by clicking Enable on `community.mermaid`; reproducible on every entry in `DEFAULT_OFF_PLUGINS` (catalog-registered shell-side plugins). Root cause: `SettingsPanelView.tsx`'s `handleToggle` (line 2855, dating from 2026-04-19 commit cd5687645) unconditionally invokes the kernel's `set_plugin_enabled` Tauri command, but the kernel only knows about WASM plugins discovered through `scan_plugin_directory`. Catalog plugins live entirely TypeScript-side via `shell/src/plugins/catalog.ts` and have no kernel-side identity. Fix: fork the toggle path — if the plugin id is in the catalog's known set, toggle via the catalog's enable/disable + persisted-prefs path (`plugins.enabled` config key, already used by the catalog loader at boot); otherwise route to the kernel as today. Likely 1–2 hours once the catalog exposes a `setEnabled(id, on)` surface that emits a fresh `plugins:status` event for the Settings panel to refresh against.
- [ ] **107 src-colocated `*.test.ts` files appear to be silently uncounted by `pnpm --filter nexus-shell test`.** Discovered 2026-05-13 during BL-110: the test script is `node --import tsx --import ./tests/setup/happy-dom.ts --test tests/*.test.ts` — a top-level `tests/*.test.ts` glob that does not descend into `src/`. After relocating BL-109's `flattenTree.test.ts` and BL-110's `frameSnapshot.test.ts` from `src/` → `shell/tests/`, the suite jumped 1246 → 1260 (the missing 14). `find shell/src -name "*.test.ts*" | wc -l` returns 107; `ls shell/tests/*.test.*` returns 96. Some src-colocated files (e.g. `gitGutter.test.ts`, `editorStore.test.ts`, `databaseViewFormat.test.ts`) may have a `tests/`-side wrapper or are typecheck-only via `tsconfig.test.json`; others may genuinely be silently skipped. Spike: per-file audit — for each src `*.test.ts`, check whether a `tests/*.test.ts` re-imports its assertions, run the file directly via `node --import tsx --test <path>`, and either (a) move it under `shell/tests/`, (b) add a thin wrapper, or (c) document why it's intentionally excluded. Likely a mix of all three. Material for correctness — closure notes that cite "N tests pass" may have been over-counting coverage for months.
- [ ] **`about:srcdoc` "frame is sandboxed and 'allow-scripts' permission is not set" console errors at boot.** Observed 2026-05-13 in the Tauri WebKit webview after the BL-109 sandbox-orchestrated boot path runs (`[CommunityLoader] 2 plugin(s) discovered (2 from repo)`). `SandboxOrchestrator.ts:324` does set `sandbox="allow-scripts"`, so the orchestrator-mounted iframes shouldn't trip this — suspicion is one of the other srcdoc consumers (`shell/src/plugins/nexus/editor/EditorView.tsx`, `shell/src/host/communityPluginLoader.ts`) is either omitting the sandbox attribute or assigning `srcdoc` before the attribute, which under WebKit means the document loads with the default deny-everything sandbox flags. Errors are noisy but non-fatal — plugins still boot to active. Spike: confirm which iframe(s) emit by adding a `data-nexus-iframe-source` attribute to each call site, reproduce in `pnpm tauri:dev`, then either set `sandbox` *before* `srcdoc` or switch to a `Blob`-URL `<iframe src=…>` so the attribute is committed before navigation. Unrelated to BL-109 itself; surfaced during BL-109 verification.
