...
Cron + file_event + git_event triggers + condition evaluator + variable interpolation + manual executor + per-step retry/backoff + parallel step scheduler are all live. Outstanding: webhook trigger (HTTP listener), mcp_event trigger, AI step types (`ai_prompt`, `ai_decision`), and a built-in workflow templates library. Track as one umbrella; mint sub-IDs when prioritised.
