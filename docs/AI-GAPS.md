...
### Follow-up (not blocking)
- ✅ **Token-level streaming** — shipped 2026-05-09. `submit_ai` is now non-blocking: it generates a session id, subscribes to the `com.nexus.ai.stream_*` topic prefix on the kernel bus, spawns the `stream_ask` IPC call onto `self.rt`, and returns immediately after inserting an empty placeholder assistant message. New `pump_ai` method (called from the render loop on every tick when a session is live) `try_recv`s pending events, appends each chunk's text onto the placeholder via `parse_chunk_event`, and harvests the IPC join handle's result via `is_finished` + `block_on`. The status line flips from "thinking…" to "streaming…" once the first chunk lands. The `Runtime.context` blocker called out earlier was sidestepped by wrapping `TuiApp.runtime` in `Arc<Runtime>` (single-line refactor; existing helpers continue to work via deref coercion); the spawned future captures an Arc clone so it owns its borrow for the call's full lifetime. New `streaming.rs` module with 12 pure tests covers the chunk/done/start parsers (session-id filtering, missing-field tolerance, empty-chunk passthrough, kernel topic-name pinning).
- ✅ **Multi-turn context** — shipped 2026-05-08. The TUI now dispatches through `com.nexus.ai::stream_ask` with the full in-memory transcript packed into the `messages: [{role, content}]` payload, so the model sees prior user/assistant turns alongside RAG-grounded retrieval. The handler treats the last `user` message as the retrieval question; system + assistant turns flow to the chat provider verbatim. The synchronous `block_on` shape stays intact — token-level streaming would need an `Arc<KernelPluginContext>` refactor and is tracked separately. New `extract_stream_ask_text` helper accepts the new `text` shape, falls back to the legacy `answer` field for forward-compat, and accepts a bare string for kernel error paths. 4 tests cover the round-trip + fallbacks.
