...
# ── Inter ────────────────────────────────────────────────────────────────────
INTER_BASE="https://github.com/rsms/inter/releases/download/v4.0"
for weight in Regular Medium SemiBold Bold; do
  dest="$FONTS_DIR/Inter-${weight}.woff2"
  if [[ ! -f "$dest" ]]; then
    echo "  Inter-${weight}.woff2"
    curl -fsSL "${INTER_BASE}/Inter-${weight}.woff2" -o "$dest"
  fi
done
...
# ── JetBrains Mono ───────────────────────────────────────────────────────────
JB_BASE="https://github.com/JetBrains/JetBrainsMono/releases/download/v2.304"
for weight in Regular Medium; do
  dest="$FONTS_DIR/JetBrainsMono-${weight}.woff2"
  if [[ ! -f "$dest" ]]; then
    echo "  JetBrainsMono-${weight}.woff2"
    curl -fsSL "${JB_BASE}/JetBrainsMono-${weight}.woff2" -o "$dest"
  fi
done
