/**
 * WI-05 — re-export wrapper so the default `pnpm test` glob
 * (`tests/*.test.ts`) picks up the saved-commands store unit tests
 * that live as a sibling of the implementation under
 * `shell/src/plugins/nexus/terminal/savedCommandsStore.test.ts`.
 *
 * Same shim pattern as `ai-store.test.ts` — node:test discovers
