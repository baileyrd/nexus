...
// BL-142 Phase 2a — REPL kernel registry: schema key + default JSON
// consumed by the manifest's schema registration block.
import {
  CONFIG_REPL_KERNELS,
...
// R8 / #191 — constants, stub-command metadata, and small pure helpers
// lifted to a sibling so this file stays focused on the manifest +
// `activate()` wiring.
import {
  applyEditorCssVars,
  COMMAND_CLOSE_ALL,
  ...
  COMMAND_NEW_UNTITLED,
  COMMAND_OPEN_DEFAULT_APP,
  COMMAND_OPEN_DIFF,
  COMMAND_REPLACE,
  COMMAND_REVEAL_IN_NAV,
  COMMAND_REVEAL_IN_OS,
  COMMAND_SAVE,
  COMMAND_TOGGLE_BLAME,
  COMMAND_TOGGLE_MODE,
  COMMAND_TOGGLE_READING_VIEW,
  CONFIG_CODE_FILE_EXTENSIONS,
  CONFIG_CONFIRM_CLOSE_DIRTY,
  CONFIG_DEFAULT_MODE,
  CONFIG_EDIT_PREDICTION_DEBOUNCE_MS,
  CONFIG_EDIT_PREDICTION_ENABLED,
  CONFIG_EDIT_PREDICTION_MODEL,
  CONFIG_EDIT_PREDICTION_PROVIDER,
  CONFIG_FONT_SIZE,
  CONFIG_KEYBINDINGS,
  CONFIG_LINE_HEIGHT,
  CONFIG_LINE_NUMBERS,
  ...
  async activate(api: PluginAPI) {
    ...
    api.configuration.register({
      ...
      schema: [
        ...
        {
          key: CONFIG_WORD_WRAP,
          title: 'Word wrap',
          description: 'Soft-wrap long lines in source and code modes. Applied live to open tabs.',
          type: 'boolean',
          default: true,
        },
        {
          key: CONFIG_TAB_SIZE,
          title: 'Tab size',
          description: 'Number of columns a tab character occupies in source and code modes. Applied live to open tabs.',
          type: 'number',
          default: 4,
        },
        {
