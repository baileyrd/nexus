  ...
  return (
    <div
      style={{
        ...
        background: 'var(--background-secondary, #252529)',
        border: `1px solid ${isDragOver ? 'var(--interactive-accent, #60a5fa)' : 'var(--background-modifier-border, #2a2a2e)'}`,
        borderRadius: 6,
        minWidth: 0,
      }}
      onDragOver={(e) => {
    ...
    >
      <div
        style={{
          ...
          fontSize: 11,
          fontWeight: 600,
          color: columnKey === UNASSIGNED ? 'var(--text-faint, #6b7280)' : 'var(--text-normal, #e4e4e7)',
          borderBottom: '1px solid var(--background-modifier-border, #2a2a2e)',
          display: 'flex',
      ...
      >
        <span>{label}</span>
        <span style={{ color: 'var(--text-faint, #6b7280)' }}>{records.length}</span>
      </div>
      <div
        style={{
          flex: 1,
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 500, marginBottom: rest.length ? 4 : 0 }}>
        {title || <span style={{ color: 'var(--text-faint, #6b7280)' }}>Untitled</span>}
      </div>
      {rest.map((c) => {
